...
const mainNav: { to: string; label: string; Icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { to: '/app/support', label: '用量与费用', Icon: Coins },
]

const toolNav: { to: string; label: string; Icon: React.ComponentType<{ className?: string }> }[] = [
  { to: '/app/compliance/system', label: '店铺', Icon: Radar },
  { to: '/app/products', label: '产品', Icon: PackageCheck },
  { to: '/app/orders', label: '销售订单', Icon: ShoppingCart },
  { to: '/app/logistics', label: '物流追踪', Icon: Truck },
  { to: '/app/news-monitor', label: '新闻监控', Icon: Newspaper },
  { to: '/app/scheduled-tasks', label: '监控任务', Icon: CalendarClock },
  { to: '/app/nl-store', label: '记忆库', Icon: Brain },
]

// 管理员配置项（智能体/技能/工具/模型/通知/第三方平台/用户管理）已收敛到
// 「系统管理」入口页（SettingsPage），侧边栏不再平铺，避免导航项过多。
