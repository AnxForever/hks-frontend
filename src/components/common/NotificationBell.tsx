...
export function NotificationBell({ className }: { className?: string }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-[min(92vw,380px)] p-0" hideClose>
        <SheetHeader className="border-b border-border/60 px-4 py-3">
          <SheetTitle className="flex items-center justify-between text-[15px]">
            ...
            <div className="flex items-center gap-0.5">
              ...
              <SheetClose asChild>
                <Button variant="ghost" size="icon" className="ml-1 h-7 w-7 text-muted-foreground/70 hover:text-foreground">
                  <X className="size-4" />
                </Button>
              </SheetClose>
            </div>
          ...
          {isLoading ? (
            ...
            <ul className="divide-y divide-border/60">
              {list.map((n: NotificationItem) => {
                ...
                return (
                  <li key={String(n.id)} className="group/notif relative flex items-start">
                    <button
                      type="button"
                      onClick={() => !read && markRead.mutate(String(n.id))}
                      className="flex min-w-0 flex-1 gap-2.5 px-4 py-3 text-left transition-colors hover:bg-muted/40"
                    >
                      <span
                        className={cn(
                    ...
                    <button
                      ...
                      onClick={() => del.mutate(String(n.id))}
                      className="mr-2 mt-2.5 shrink-0 rounded-md p-1.5 text-muted-foreground/40 opacity-0 transition-all hover:bg-muted/60 hover:text-destructive focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring group-hover/notif:opacity-100"
                    >
                      <Trash2 className="size-3.5" />
                    </button>
                  </li>
