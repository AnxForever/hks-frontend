...
export function useUpdateProduct() {
  ...
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ productId, ...body }: { productId: string } & Partial<ProductCreateBody>) =>
      productsApi.update(authFetch, productId, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: productKeys.all }),
  })
}
...
export function useProductComplianceHistory(productId: string | undefined) {
  const { authFetch } = useAuth()
  return useQuery({
    queryKey: ['products', productId, 'compliance-history'],
    queryFn: () => productsApi.complianceHistory(authFetch, productId!),
