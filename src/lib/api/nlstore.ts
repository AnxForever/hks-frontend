 ...
 */
import type { NLAllItem, NLRecord, NLRecordCreateRequest, NLSearchResult, NLSummaryItem } from '@/types'

const BASE = '/api/v1/nl-store'

/** 全量记录列表（所有 namespace，含 namespace 字段） */
export async function listAllNLRecords(
...
/** 命名空间列表 */
export async function listNLNamespaces(
  authFetch: (input: RequestInfo, init?: RequestInit) => Promise<Response>,
): Promise<string[]> {
  const res = await authFetch(`${BASE}/namespaces`)
  if (!res.ok) throw new Error(`获取命名空间失败: HTTP ${res.status}`)
