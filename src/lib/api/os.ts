...
export const productsApi = {
  ...
  update: (authFetch: AuthFetch, productId: string, body: Partial<ProductCreateBody>) =>
    request<ProductItem>(authFetch, `${API}/products/${productId}`, {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  updateLifecycle: (
  ...
  complianceHistory: (authFetch: AuthFetch, productId: string) =>
    request<{ product_id: string; checks: ComplianceRecordItem[] }>(
      authFetch,
      `${API}/products/${productId}/compliance`,
    ),
  deleteComplianceRecord: (authFetch: AuthFetch, productId: string, checkId: string) =>
    request<{ success: boolean }>(authFetch, `${API}/products/${productId}/compliance/${checkId}`, {
      method: 'DELETE',
    }),
}
...
import type { ProductTodo } from '@/types'
...
export const productTodosApi = {
  list: (authFetch: AuthFetch, productId: string, status?: string) =>
    request<{ todos: ProductTodo[]; total: number }>(
      authFetch,
      `${API}/products/${productId}/todos${status ? `?status=${status}` : ''}`,
    ),
  create: (authFetch: AuthFetch, productId: string, body: TodoCreateBody) =>
    request<ProductTodo>(authFetch, `${API}/products/${productId}/todos`, {
      method: 'POST',
      body: JSON.stringify(body),
    }),
  update: (authFetch: AuthFetch, productId: string, todoId: string, body: Partial<{ status: string; title: string; priority: string }>) =>
    request<ProductTodo>(authFetch, `${API}/products/${productId}/todos/${todoId}`, {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  execute: (authFetch: AuthFetch, productId: string, todoId: string) =>
    request<{ success: boolean; status: string; result_summary?: string }>(
      authFetch,
      `${API}/products/${productId}/todos/${todoId}/execute`,
      { method: 'POST' },
    ),
  delete: (authFetch: AuthFetch, productId: string, todoId: string) =>
    request<{ success: boolean }>(authFetch, `${API}/products/${productId}/todos/${todoId}`, {
      method: 'DELETE',
    }),
}
...
export const integrationsApi = {
  ...
  updateConfig: (authFetch: AuthFetch, id: string, config: Record<string, unknown>, label?: string) =>
    request<IntegrationConnection>(authFetch, `${API}/integrations/${id}/config`, {
      method: 'PUT',
      body: JSON.stringify({ config, ...(label !== undefined ? { label } : {}) }),
    }),
