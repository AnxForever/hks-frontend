...
export default function Dashboard() {
  ...
  // R8: token 用量 → 预计节省成本锚点卡（数据源 /model-configs/usage）
  ...
  useEffect(() => {
    ...
    modelConfigsApi
      .getUsage()
      .then((data) => {
        if (!cancelled) setUsageTokens(data.total_tokens ?? 0)
      })
      .catch(() => {
        if (!cancelled) setUsageTokens(null)
      })
    return () => {
      cancelled = true
    }
  ...
  // L9: 健康分单一事实源 — 统一使用 /metrics/dashboard 的 health_score
  const healthScore = dashboardMetrics?.health_score != null ? Math.round(dashboardMetrics.health_score) : '—'
  ...
  // R8: 预计节省 = 合规检查次数 × 外包单次市价 − 平台用量折算，另计 token 直调差价
  const totalChecks = useMemo(
    () => (dashboardMetrics?.trend ?? []).reduce((sum, d) => sum + (d.checks ?? 0), 0),
    [dashboardMetrics?.trend],
  )
  const estimatedSaved = useMemo(() => {
    if (usageTokens === null) return null
    const outsourceSaved = totalChecks * OUTSOURCE_PRICE_CNY_PER_QUERY - platformCostCNY(usageTokens)
    const llmSaved = savedCNY(usageTokens)
    return Math.max(outsourceSaved, llmSaved, 0)
  }, [usageTokens, totalChecks])
    ...
    <div className="h-full overflow-y-auto bg-background">
      ...
      <div className="mx-auto max-w-[1400px] space-y-8 px-6 py-8 sm:px-8">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
          ...
          <Metric
            label="预计节省成本"
            value={estimatedSaved !== null ? formatCNY(estimatedSaved) : '—'}
            detail={`按 ${totalChecks} 次合规检查对比外包咨询估算`}
            Icon={PiggyBank}
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {TARGET_MARKETS.map((m) => (
                <button
                  key={m.code}
                  onClick={() =>
                    navigate('/app/products', { state: { country: m.label } })
                  }
        ...
        {shopsLoading ? (
          ...
          <section>
            <div className="mb-4">
              ...
              <p className="mt-1 text-[13px] text-muted-foreground">点击进入产品工作台开展合规检查</p>
            </div>
            <ErrorState
              title="店铺加载失败"
          ...
          <section>
            <div className="mb-4 flex items-end justify-between gap-4">
              ...
              <Button
                ...
                onClick={() => navigate('/app/products')}
              >
                产品 <ArrowRight className="ml-1 size-3.5" />
              </Button>
            </div>
