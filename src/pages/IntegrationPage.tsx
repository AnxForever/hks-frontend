...
// 集成页仅管理数据/电商平台；IM 平台（飞书/钉钉/企微/Slack）属通知渠道，在「通知」页配置
const IM_PROVIDERS = new Set(['feishu', 'dingtalk', 'wecom', 'slack'])
...
export default function IntegrationPage() {
  ...
  const rawProviders = dashboard.data?.providers?.length ? dashboard.data.providers : fallbackProviders
  const providers = rawProviders.filter((p) => !IM_PROVIDERS.has(p.provider))
  ...
  const connectedCount = Object.entries(providerStatus).filter(
    ([key, s]) => !IM_PROVIDERS.has(key) && s?.status === 'connected',
  ).length
    ...
    <div className="h-full overflow-y-auto bg-background">
      ...
      <div className="mx-auto max-w-[1400px] space-y-7 px-6 py-8 sm:px-8">
        <section className="grid gap-3 md:grid-cols-3">
          <Summary label="平台模板" value={providers.length} detail="后端可用平台类型" Icon={Cable} />
      ...
      <ConfigDialog
        ...
        onSubmit={async (id, config, label) => {
          await updateConfig.mutateAsync({ id, config, label })
          toast.success('配置已更新')
          setEditing(null)
        }}
      />
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent>
        ...
        <form className="space-y-4" onSubmit={submit}>
          <div>
            <label htmlFor={providerId} className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">平台类型</label>
            <select
              id={providerId}
              name="provider"
              value={provider}
...
function ConfigDialog({
  ...
  saving: boolean
  onClose: () => void
  onSubmit: (id: string, config: Record<string, unknown>, label?: string) => Promise<void>
}) {
  const [label, setLabel] = useState('')
  const [configText, setConfigText] = useState('{}')
  const [jsonError, setJsonError] = useState('')
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setJsonError('')
    await onSubmit(connection.id, config, label.trim())
    setConfigText('{}')
  }
