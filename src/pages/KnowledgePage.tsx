...
const USE_BACKEND_KNOWLEDGE = import.meta.env.VITE_STREAM_MODE !== 'mock'
...
/** 分类 → 展示样式映射（真实文档派生精选卡片时使用） */
const CATEGORY_STYLE: Record<Exclude<CategoryId, 'all'>, Pick<FeaturedArticle, 'label' | 'Icon' | 'tone'>> = {
  compliance: { label: '合规法规', Icon: ShieldCheck, tone: 'blue' },
  tax: { label: '关税税务', Icon: Landmark, tone: 'amber' },
  certification: { label: '认证流程', Icon: Gavel, tone: 'red' },
  cases: { label: '案例分析', Icon: BookOpenCheck, tone: 'emerald' },
  industry: { label: '行业报告', Icon: Globe2, tone: 'violet' },
}

/** 根据文档名推断分类（后端文档模型无 category 字段） */
function inferCategory(doc: KnowledgeDoc): Exclude<CategoryId, 'all'> {
  const text = doc.name.toLowerCase()
  if (/认证|\b(ce|fcc|pse|ul)\b/.test(text)) return 'certification'
  if (/税|vat|ioss|tariff/.test(text)) return 'tax'
  if (/案例|case/.test(text)) return 'cases'
  if (/法规|合规|regulation|gpsr|reach|compliance/.test(text)) return 'compliance'
  return 'industry'
}

/** 真实文档 → 精选卡片 */
function docToArticle(doc: KnowledgeDoc): FeaturedArticle {
  ...
  return {
    ...
    title: doc.name,
    description:
      doc.doc_type === 'url' && doc.source_url
        ? doc.source_url
        : `${doc.market.toUpperCase()} 市场文档 · ${doc.chunk_count || 0} 个片段`,
    meta: `${doc.doc_type.toUpperCase()} · ${new Date(doc.created_at * 1000).toLocaleDateString('zh-CN')}`,
  }
}

/** 示例数据：仅在显式 mock 模式（VITE_STREAM_MODE=mock）下展示 */
const SAMPLE_ARTICLES: FeaturedArticle[] = [
  {
    category: 'compliance',
    label: '合规法规',
...
export default function KnowledgePage() {
  ...
  // 精选列表：真实模式从已就绪文档派生最新 6 篇；mock 模式保留示例数据
  const featuredArticles = useMemo<FeaturedArticle[]>(() => {
    if (!USE_BACKEND_KNOWLEDGE) return SAMPLE_ARTICLES
    return (docs ?? [])
  ...
  }, [featuredArticles, category, query])
    ...
    <div className="h-full overflow-y-auto bg-muted/40 px-4 py-5 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-7">
          ...
          {filteredArticles.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border bg-card p-8 text-center text-[13px] text-muted-foreground lg:col-span-3">
              {featuredArticles.length === 0
                ? '知识库暂无已就绪文档，可通过下方 PDF / URL 导入'
                : '未找到匹配内容'}
            </div>
          ) : (
            filteredArticles.map((article, index) => (
              <KnowledgeCard key={`${article.title}-${index}`} article={article} />
            ))
          )}
        </section>
        ...
        <section aria-label="推荐服务商">
          ...
          {/* 服务商入驻体系搭建中，暂无真实数据，先以诚实占位替代原虚构数据 */}
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/70 bg-card/50 px-6 py-10 text-center">
            <div className="mb-2 text-[28px]" aria-hidden="true">🤝</div>
            <p className="text-[14px] font-medium text-foreground">服务商入驻体系搭建中</p>
            <p className="mt-1 max-w-md text-[12px] leading-relaxed text-muted-foreground">
              我们正在引入经过审核的税务、认证与法律服务商，确保推荐真实可信。功能上线后将在此展示。
            </p>
          </div>
        </section>
