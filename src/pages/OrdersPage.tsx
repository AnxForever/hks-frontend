...
export default function OrdersPage() {
  ...
  // 多币种不可直接相加：按币种分组汇总，分别展示。
  const totalsByCurrency = filtered.reduce<Record<string, number>>((acc, o) => {
    const cur = o.currency || 'USD'
    acc[cur] = (acc[cur] ?? 0) + (o.total_amount || 0)
    return acc
  }, {})
    ...
    <div className="h-full overflow-y-auto bg-background">
      ...
      <div className="mx-auto max-w-[1400px] space-y-4 px-6 py-6 sm:px-8">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <SummaryCard label="订单总额（按币种）" value={totalText} icon={Package} />
