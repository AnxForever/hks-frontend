...
const ProductTodoPage = lazy(() => import('@/pages/ProductTodoPage'))
...
// Admin 配置页 - 懒加载
const ConfigLayout = lazy(() => import('@/components/config/ConfigLayout'))
...
const SkillsManagePage = lazy(() => import('@/pages/config/SkillsManagePage'))
const ToolsManagePage = lazy(() => import('@/pages/config/ToolsManagePage'))
 ...
 * ── /support      兼容入口 → /app/support
 * ── /payment      兼容入口 → /app/support
 ...
 * ── /app/user-manage 仅管理员（RequireAdmin）；其余配置页成员与管理员一致可见，
 *    系统级 skill/工具条目由后端按角色过滤
 * ── *             NotFoundPage（友好 404）
 */
...
export const router = createBrowserRouter([
  ...
  {
    path: '/support',
    element: <Navigate to="/app/support" replace />,
  },
  {
    path: '/payment',
    element: <Navigate to="/app/support" replace />,
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'products/:productId/todos', element: <Lazy><ProductTodoPage /></Lazy> },
      ...
      {
        // 历史入口：只读能力目录已并入统一的 Skills 管理页
        // （成员可见，系统级技能条目由后端按角色过滤）
        path: 'capabilities',
        element: <Navigate to="/app/skills" replace />,
      },
      {
        // 配置页：成员与管理员一致可见（仅需登录）；
        // 写操作由后端 require_admin 兜底
        element: (
          <Lazy>
            <ConfigLayout />
          </Lazy>
